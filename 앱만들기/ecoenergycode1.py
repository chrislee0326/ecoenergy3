...
#탄소 배출량 계산하기
#석탄발전888 천연가스499 톤/GWh
co2 = 888*int(coal)+499*int(gas)
co2 = 0.001*co2
mvco2 = int(co2)/int(total)
...
st.write("")
st.write("## <탄소 배출량 정보>")
st.write("##### 탄소배출량: "+ str(round(co2,1)) +"t" )
st.write("##### 단위 전력(MV)당 탄소배출량"+str(round(mvco2,2))+"t")
st.write("")
if st.checkbox('자세히 보기 및 세로 고침'):
    st.write('날짜/시간: '+ str(time))
    st.write('전체발전량: '+ str(total)+'MW')
    st.write('신재생에너지: '+str(eco)+'MW')
    st.write('가스: '+str(gas)+'MW')
    st.write('화력발전(석탄발전): '+str(coal)+'MW')
    st.write('원자력발전: '+str(nuclear)+'MW')
    st.write('기타: '+str(another2)+'MW')
st.write("*매우나쁨 일 때는 이렇게 행동하세요! : 인쇄 작업, 난방, 샤워 등 일시적으로 전기를 사용해야하는 행동을 자제하세요. 이후 등급이 올라갔을 때 미루었던 일을 마무리하시기 바랍니다.")
st.write("*이 앱은 한국 전력 거래소 사이트의 실시간 전력수급현황_발전원별 발전량 데이터를 기반으로 합니다.")
st.write("*비율계산식: (신재생에너지 발전량/전체 발전량)*100(%)")
st.write("등급 안내")
st.write("매우좋음: 12%이상")
st.write("좋음: 10%이상, 12%미만")
st.write("보통: 8%이상, 10%미만")
st.write("나쁨: 6%이상, 8%미만")
st.write("매우나쁨: 6%미만")
